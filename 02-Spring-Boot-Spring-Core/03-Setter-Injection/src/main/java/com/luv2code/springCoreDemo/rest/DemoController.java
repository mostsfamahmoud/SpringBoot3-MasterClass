...
public class DemoController {
    ...
    // Constructor injection could be used, but here we're using setter injection
    // You can also inject dependencies using a setter method (demonstrated below).

    //// Setter Injection (if used)
    // @Autowired
    // public void setCoach(Coach theCoach) {
    //     myCoach = theCoach;
    // }

    // Setter injection using a method with a custom name (method injection)
    @Autowired  // Tells Spring to automatically inject a Coach implementation into this method
    public void doSomeStuff(Coach theCoach) {
        myCoach = theCoach;  // Assign the injected Coach object to the private field
    }
